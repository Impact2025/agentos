...
class NotebookLMClient:
    ...
    def connect(self) -> None:
        """MCP initialize + initialized-notification. Eenmalig per client."""
        ...
        if not sid:
            raise NotebookLMError("Geen Mcp-Session-Id ontvangen van notebooklm-mcp")
        self._sid = sid
    ...
    def close(self) -> None:
        if self._sid:
            try:
                self._session.delete(f"{self.base}/mcp", headers=self._headers(),
                                     timeout=10)
            except Exception:
                pass
            self._sid = None
    ...
    def ask(self, question: str, notebook_id: Optional[str] = None,
        ...
        return {
            ...
            "raw": "" if "answer" in data else self._call("ask_question", args),
        }
