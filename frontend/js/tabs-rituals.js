...
function loadRituelenSection() {
  ...
  Promise.all([
    ...
    el.innerHTML = '<div style="color:#ef4444">Rituelen laden mislukt: ' + escHtml(e.message) + '</div>';
  });
}

function _ritBadge(done, label) {
  var bg = done ? '#f0fdf4' : '#fef2f2', fg = done ? '#16a34a' : '#b91c1c';
  var mark = done ? '✓' : '–';
  return '<span style="display:inline-flex;align-items:center;gap:4px;padding:3px 9px;border-radius:12px;' +
    'background:' + bg + ';color:' + fg + ';font-size:11px;font-weight:600">' + mark + ' ' + escHtml(label) + '</span>';
}
...
function renderRituelenOverview(el, status, wins, goals) {
  ...
  var html = '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px">' +
    ...
    '<span style="font-size:11px;color:#7c3aed;font-weight:600;margin-left:4px">\u{1F525} ' +
    (streaks.morning || 0) + 'd ochtend · ' + (streaks.evening || 0) + 'd avond</span></div>';

  html += '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px">';
  html += '<button onclick="showMorningForm()" style="padding:5px 12px;background:' +
    (today.morning_done ? '#f1f5f9' : '#16a34a') + ';color:' + (today.morning_done ? '#475569' : '#fff') +
  ...
  html += '<button onclick="showEveningForm()" style="padding:5px 12px;background:' +
    (today.evening_done ? '#f1f5f9' : '#0f172a') + ';color:' + (today.evening_done ? '#475569' : '#fff') +
    ';border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer">' +
    (today.evening_done ? 'Avondritueel bekijken' : 'Avondritueel starten') + '</button>';
  html += '<button onclick="showWeeklyStartForm()" style="padding:5px 12px;background:#f1f5f9;color:#475569;' +
    'border:1px solid #e2e8f0;border-radius:6px;font-size:11px;cursor:pointer">Weekstart</button>';
  html += '<button onclick="showWeeklyReviewForm()" style="padding:5px 12px;background:#f1f5f9;color:#475569;' +
    'border:1px solid #e2e8f0;border-radius:6px;font-size:11px;cursor:pointer">Weekreview</button>';
  html += '</div>';

  // Wins
  html += '<div style="margin-bottom:14px"><div style="display:flex;align-items:center;justify-content:space-between">' +
    '<h5 style="font-size:12px;font-weight:700;color:#0f172a;margin:0">\u{1F3C6} Wins (Cookie Jar)</h5>' +
    '<button onclick="showAddWinForm()" style="padding:2px 9px;background:#fef3c7;color:#92400e;border:none;' +
    'border-radius:5px;font-size:10px;font-weight:600;cursor:pointer">+ Win</button></div>';
  if (!wins.length) {
    ...
    wins.forEach(function (w) {
      html += '<div style="font-size:11px;padding:5px 8px;background:#fffbeb;border-radius:6px;display:flex;' +
        'justify-content:space-between;gap:8px"><span>' + escHtml(w.title) + '</span>' +
        '<span style="color:#b45309;white-space:nowrap">' + escHtml(w.date) + '</span></div>';
    });
    html += '</div>';
  }
  html += '</div>';
  ...
  html += '<div><div style="display:flex;align-items:center;justify-content:space-between">' +
    '<h5 style="font-size:12px;font-weight:700;color:#0f172a;margin:0">\u{1F3AF} Persoonlijke doelen</h5>' +
    '<button onclick="showAddGoalForm()" style="padding:2px 9px;background:#ede9fe;color:#6d28d9;border:none;' +
    'border-radius:5px;font-size:10px;font-weight:600;cursor:pointer">+ Doel</button></div>';
  if (!goals.length) {
    ...
    goals.forEach(function (g) {
      html += '<div style="font-size:11px;padding:6px 8px;background:#f5f3ff;border-radius:6px">' +
        '<div style="display:flex;justify-content:space-between;gap:8px">' +
        '<span style="font-weight:600">' + escHtml(g.title) + '</span>' +
        '<span style="color:#7c3aed">' + g.progress + '%</span></div>' +
        '<div style="height:4px;background:#ede9fe;border-radius:2px;margin-top:4px">' +
        '<div style="height:100%;width:' + g.progress + '%;background:#7c3aed;border-radius:2px"></div></div>' +
        '</div>';
    });
    html += '</div>';
  }
...
function showMorningForm() {
  ...
  fetch('/api/rituals/morning').then(function (r) { return r.json(); }).then(function (d) {
    ...
    el.innerHTML =
      '<div style="font-size:12px;font-weight:700;margin-bottom:8px">☀️ Ochtendritueel &mdash; vandaag</div>' +
      ...
      'border:1px solid #e2e8f0;border-radius:6px" placeholder="Vandaag focus ik op...">' + escHtml(d.intentie || '') + '</textarea>' +
      '<div style="display:flex;gap:6px;margin-bottom:8px">' +
      '<div style="flex:1"><label style="font-size:11px;color:#64748b">Focusblok 1</label>' +
      '<input id="rit-m-fb1" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;border-radius:6px" ' +
      ...
      '<input id="rit-m-fb2" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;border-radius:6px" ' +
      ...
      '" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;border-radius:6px"></div>' +
      ...
      '" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;border-radius:6px"></div></div>' +
      ...
      [0, 1, 2].map(function (i) {
        return '<input id="rit-m-dank' + i + '" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;' +
      ...
      'border:1px solid #e2e8f0;border-radius:6px" placeholder="Ik ben... Ik heb... Ik bereik...">' +
      escHtml(d.affirmatie || '') + '</textarea>' +
      '<div style="display:flex;gap:6px">' +
      '<button onclick="saveMorningForm()" style="padding:6px 14px;background:#16a34a;color:#fff;border:none;' +
      'border-radius:6px;font-size:12px;font-weight:600;cursor:pointer">Opslaan</button>' +
      '<button onclick="loadRituelenSection()" style="padding:6px 14px;background:#f1f5f9;color:#475569;' +
      'border:1px solid #e2e8f0;border-radius:6px;font-size:12px;cursor:pointer">Terug</button></div>';
  });
}
...
function saveMorningForm() {
  ...
  post('/api/rituals/morning', body).then(loadRituelenSection).catch(function (e) { alert(e.message); });
}
...
function showEveningForm() {
  ...
  fetch('/api/rituals/evening').then(function (r) { return r.json(); }).then(function (d) {
    ...
    el.innerHTML =
      '<div style="font-size:12px;font-weight:700;margin-bottom:8px">\u{1F319} Avondritueel &mdash; vandaag</div>' +
      ...
      '" style="width:100%;margin:2px 0 8px;font-size:12px;padding:5px;border:1px solid #e2e8f0;border-radius:6px">' +
      '<label style="font-size:11px;color:#64748b">Top 3 voor morgen</label>' +
      '<div style="display:flex;flex-direction:column;gap:4px;margin:2px 0 8px">' +
      [0, 1, 2].map(function (i) {
        return '<input id="rit-e-top' + i + '" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;' +
      ...
      '<button onclick="saveEveningForm()" style="padding:6px 14px;background:#0f172a;color:#fff;border:none;' +
      'border-radius:6px;font-size:12px;font-weight:600;cursor:pointer">Opslaan</button>' +
      '<button onclick="loadRituelenSection()" style="padding:6px 14px;background:#f1f5f9;color:#475569;' +
      'border:1px solid #e2e8f0;border-radius:6px;font-size:12px;cursor:pointer">Terug</button></div>';
  });
...
function _ritField(id, label, value, textarea, marginSmall) {
  var tag = textarea
    ? '<textarea id="' + id + '" rows="2" style="width:100%;font-size:12px;padding:6px;border:1px solid #e2e8f0;' +
      'border-radius:6px">' + escHtml(value || '') + '</textarea>'
    : '<input id="' + id + '" style="width:100%;font-size:12px;padding:6px;border:1px solid #e2e8f0;' +
...
function saveEveningForm() {
  ...
  post('/api/rituals/evening', body).then(loadRituelenSection).catch(function (e) { alert(e.message); });
}
...
function showWeeklyStartForm() {
  ...
  fetch('/api/rituals/weekly-start').then(function (r) { return r.json(); }).then(function (d) {
    ...
    el.innerHTML =
      '<div style="font-size:12px;font-weight:700;margin-bottom:8px">\u{1F680} Weekstart</div>' +
      ...
      goals.map(function (g, i) {
        return '<input class="rit-ws-goal" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;' +
      ...
      '<div style="display:flex;gap:6px">' +
      '<button onclick="saveWeeklyStartForm()" style="padding:6px 14px;background:#16a34a;color:#fff;border:none;' +
      'border-radius:6px;font-size:12px;font-weight:600;cursor:pointer">Opslaan</button>' +
      '<button onclick="loadRituelenSection()" style="padding:6px 14px;background:#f1f5f9;color:#475569;' +
      'border:1px solid #e2e8f0;border-radius:6px;font-size:12px;cursor:pointer">Terug</button></div>';
  });
...
function saveWeeklyStartForm() {
  ...
  post('/api/rituals/weekly-start', body).then(loadRituelenSection).catch(function (e) { alert(e.message); });
}
...
function showWeeklyReviewForm() {
  ...
  fetch('/api/rituals/weekly-review').then(function (r) { return r.json(); }).then(function (d) {
    ...
    el.innerHTML =
      '<div style="font-size:12px;font-weight:700;margin-bottom:8px">\u{1F3C6} Weekreview</div>' +
      ...
      '</div>' +
      '<div style="display:flex;gap:6px">' +
      '<button onclick="saveWeeklyReviewForm()" style="padding:6px 14px;background:#16a34a;color:#fff;border:none;' +
      'border-radius:6px;font-size:12px;font-weight:600;cursor:pointer">Opslaan</button>' +
      '<button onclick="loadRituelenSection()" style="padding:6px 14px;background:#f1f5f9;color:#475569;' +
      'border:1px solid #e2e8f0;border-radius:6px;font-size:12px;cursor:pointer">Terug</button></div>';
  });
...
function saveWeeklyReviewForm() {
  ...
  post('/api/rituals/weekly-review', body).then(loadRituelenSection).catch(function (e) { alert(e.message); });
}
...
function showAddWinForm() {
  ...
  if (!el) return;
  el.innerHTML =
    '<div style="font-size:12px;font-weight:700;margin-bottom:8px">\u{1F3C6} Nieuwe win</div>' +
    '<input id="rit-win-title" style="width:100%;font-size:12px;padding:6px;border:1px solid #e2e8f0;' +
    'border-radius:6px;margin-bottom:8px" placeholder="Titel">' +
    '<textarea id="rit-win-desc" rows="2" style="width:100%;font-size:12px;padding:6px;border:1px solid #e2e8f0;' +
    ...
    '<select id="rit-win-cat" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;border-radius:6px">' +
    ...
    'padding:5px;border:1px solid #e2e8f0;border-radius:6px"></div></div>' +
    '<div style="display:flex;gap:6px">' +
    '<button onclick="saveWinForm()" style="padding:6px 14px;background:#f59e0b;color:#fff;border:none;' +
    'border-radius:6px;font-size:12px;font-weight:600;cursor:pointer">Toevoegen</button>' +
    '<button onclick="loadRituelenSection()" style="padding:6px 14px;background:#f1f5f9;color:#475569;' +
    'border:1px solid #e2e8f0;border-radius:6px;font-size:12px;cursor:pointer">Terug</button></div>';
}
...
function showAddGoalForm() {
  ...
  if (!el) return;
  el.innerHTML =
    '<div style="font-size:12px;font-weight:700;margin-bottom:8px">\u{1F3AF} Nieuw persoonlijk doel</div>' +
    '<input id="rit-goal-title" style="width:100%;font-size:12px;padding:6px;border:1px solid #e2e8f0;' +
    'border-radius:6px;margin-bottom:8px" placeholder="Titel">' +
    ...
    '<select id="rit-goal-cat" style="width:100%;font-size:12px;padding:5px;border:1px solid #e2e8f0;' +
    ...
    '<div style="display:flex;gap:6px">' +
    '<button onclick="saveGoalForm()" style="padding:6px 14px;background:#7c3aed;color:#fff;border:none;' +
    'border-radius:6px;font-size:12px;font-weight:600;cursor:pointer">Toevoegen</button>' +
    '<button onclick="loadRituelenSection()" style="padding:6px 14px;background:#f1f5f9;color:#475569;' +
    'border:1px solid #e2e8f0;border-radius:6px;font-size:12px;cursor:pointer">Terug</button></div>';
}
